...
public class OrganizationUnitAppService : IdentityAppServiceBase, IOrganizationUnitAppService
{
    ...
    public virtual async Task<ListResultDto<OrganizationUnitDto>> GetRootAsync(bool recursive = false)
    {
        var rootOrganizationUnits = await OrganizationUnitManager.FindChildrenAsync(null, recursive);

        return new ListResultDto<OrganizationUnitDto>(
            ObjectMapper.Map<List<OrganizationUnit>, List<OrganizationUnitDto>>(rootOrganizationUnits));
    }

    public virtual async Task<ListResultDto<OrganizationUnitUsersDto>> GetOrgsUsersAsync(bool recursive = false)
    {
        var rootOrganizationUnits = await OrganizationUnitManager.FindChildrenAsync(null, recursive);
        ...
        foreach (var org in rootOrganizationUnits)
        {
            ...
            var organizationUnitUsers = await OrganizationUnitRepository.GetMembersAsync(org, null, int.MaxValue, 0, null);
            tmpOrg.Users = ObjectMapper.Map<List<IdentityUser>, List<IdentityUserDto>>(organizationUnitUsers);
            result.Add(tmpOrg);
        }
    ...
    public virtual async Task<PagedResultDto<OrganizationUnitDto>> GetListAsync(OrganizationUnitGetListInput input)
    {
    ...
    public virtual async Task<PagedResultDto<IdentityUserDto>> GetUnaddedUsersAsync(Guid id, OrganizationUnitGetUnaddedUserListInput input)
    {
        var organizationUnit = await OrganizationUnitRepository.GetAsync(id);
